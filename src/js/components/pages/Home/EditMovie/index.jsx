...
export class EditMovieModal extends Component {
    ...
    componentDidMount() {
        if (this.props.movie) {
            let selectedGenres = this.props.movie.genre
            let { genres } = this.state
            genres.forEach((genre, index) => {
                if (selectedGenres.indexOf(genre.value) > -1) {
                    genres[index].isChecked = true
                }
            })
            this.setState({
                ...
                popularity: this.props.movie["99popularity"],
                imdbRating: this.props.movie.imdb_score,
                genres,
            })
        }
    }
    ...
    componentDidUpdate(prevProps) {
        if (this.props.movieID !== prevProps.movieID && this.props.movie) {
            let selectedGenres = this.props.movie.genre
            let { genres } = this.state
            genres.forEach((genre, index) => {
                if (selectedGenres.indexOf(genre.value) > -1) {
                ...
                <ModalBody>
                    ...
                    <Form>
                        <FormGroup>
                            ...
                            <Input value={name} type="text" name="name" id="editName" placeholder="Name" onChange={(e) => { this.onChangeHandler(e, 'name') }} />
                        </FormGroup>
                        <FormGroup>
                            <Label for="director">Director</Label>
                            <Input value={director} type="text" name="director" id="editDirector" placeholder="Director" onChange={(e) => { this.onChangeHandler(e, 'director') }} />
                        </FormGroup>
                        <FormGroup>
                            <Label for="popularity">Popularity</Label>
                            <Input value={popularity} type="number" step="0.1" min={1.0} max={100.0} name="popularity" id="editPopularity" placeholder="Popularity" onChange={(e) => { this.onChangeHandler(e, 'popularity') }} />
                        </FormGroup> <FormGroup>
